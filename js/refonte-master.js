...
function subjectFor(kind,fd){const nom=fd.get('nom')||'',formation=fd.get('formation')||'',date=fd.get('session_date')||'',org=fd.get('organisation')||'',profil=fd.get('profil')||'';switch(kind){case'PREINSCRIPTION FORMATION':return `PREINSCRIPTION FORMATION | ${formation} | ${date} | ${nom}`;case'BESOIN RECRUTEMENT':return `BESOIN RECRUTEMENT | ${org} | ${nom}`;case'PROJET ENTREPRISE':return `PROJET ENTREPRISE | ${org} | ${fd.get('objectif')||''}`;case'PRESCRIPTEUR':return `PRESCRIPTEUR | ${org} | ${fd.get('situation')||''}`;case'ORIENTATION FINANCEMENT':return `ORIENTATION FINANCEMENT | ${nom} | ${profil}`;default:return `CONTACT | ${fd.get('objectif')||''} | ${nom}`;}}
$$('[data-sp-form]').forEach(form=>form.addEventListener('submit',async e=>{
 ...
 finally{btn.disabled=false;btn.textContent='Envoyer la demande';}
}));
})();
